...
这个好像还不是普通的降维方法， 是带上一个标签的

这里的样本距离函数，不是单纯的几何距离， 而是先用一个需要训练的函数进行处理， 然后才计算几何距离，作者为这个函数提出了一个新的损失函数

作者提出了一个新的对比损失函数， 用来在对比学习中计算loss，这个损失函数有点分类任务那味道

如果两个点是相似的，那么就通过更新参数的方式，提升样本距离
如果两个点是不相似的， 也是一样更新参数， 但是目的是降低样本距离
这个降低还是新增， 在损失函数中规定了一个特殊的格式， 会根据是否相似的二元指数Y， 来自动调整正负
然后对于样本距离函数里面的参数W， 还是通过梯度下降的方法进行处理

（其实就是正常的损失迭代的过程，可能是这篇论文年头比较长了，还要专门说明一下）
...
其中G是可以训练的函数，这里面很明显是两个子网络的输入， 再进行一个计算
子网络共享参数w， 所以这个训练应该使用娈生网络